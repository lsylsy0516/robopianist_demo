...
To mirror the piano workflow, `robopianist.models.drum.midi_module.MidiModule` watches the kit's strike geoms and emits General MIDI percussion notes whenever a contact impulse exceeds a configurable threshold. Register the module's note-on/off callbacks with a synthesizer (e.g., RoboPianist's `Synthesizer`) to hear the snare, kick, and cymbals each time they are struck. Extra percussion geoms can be mapped by supplying a custom `geom_to_note` dictionary when constructing the module.
